...
-- 고객 테이블에 'apple'고객을 삭제 했을 때, 주문 테이블에서 'apple'고객의 주문 정보가 함께 삭제되는지? (on delete cascade)
...
-- customer table에 존재하는 회원만 orders 테이블에 데이터를 추가할 수 있음
-- 만약에 orders 테이블이 있는데, customer 테이블을 삭제(drop)하면?
-- => orders 테이블은 어떤 고객의 생일 정보를 알고 싶어도 customer 테이블 자체가 없어졌기 때문에 알 수 없음
...
-- 사전순(가나다순)으로 박민지보다 뒤에 위치한 고객의 모든 정보 검색
...
-- 참고
select * from customer where custname like '%수'; -- 이름이 몇자든 마지막 글자가 '수'인 사람을 모두 선택
...
select * from customer order by custname; -- 오름차순(기본값: asc)
...
/* select custid, sum(amount), sum(price * amount) from orders 
    ...
    group by custid; -- error 1111: group 함수 잘못 사용 */

-- 총 주문액이 10000원 이상인 고객에 대해서 고객별로 주문한 상품 총 수량 구하기
-- (단, custid 'bunny'인 고객 제외하고 출력)
select custid, sum(amount), sum(price * amount) from orders 
	where custid != 'bunny' -- where도 group by와 사용 가능하나 모든 곳에 사용할 수 는 없음
	group by custid 
    having sum(price * amount) >= '10000';
